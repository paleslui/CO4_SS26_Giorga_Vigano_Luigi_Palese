...
def _seg_one(img):
    """Otsu (invert) -> close -> fill -> open -> largest touching bottom."""
    _, th = cv.threshold(img, 0, 255, cv.THRESH_BINARY_INV | cv.THRESH_OTSU)
    m = (th>0).astype(np.uint8)
    m = cv.morphologyEx(m, cv.MORPH_CLOSE, np.ones((15,15), np.uint8))   # stronger close (seals fuoc rim)
    m = fill_holes(m)
...
def _score(m):
    ...
    if a < 0.002 or a > 0.45: return -1.0
    return solidity(m) * 10 + min(a, 0.2)    # higher solidity wins; reward area in range

def segment_roi(roi, is_blank=False):
    if is_blank: return np.zeros_like(roi)
    # try BOTH polarities (defensive: her normalize_drop_polarity occasionally votes wrong)
    cand_a = _seg_one(roi)
    cand_b = _seg_one(255 - roi)
...
tiles=[]
print(f"{'image':16s} {'yb':>4s} blank {'area':>6s} {'solid':>5s} {'inv?':>4s}")
for nm in names:
    path=os.path.join(SRC,nm+".jpg")
    pre = pp.preprocess_image(path)
    roi, yb, blank = pre['roi'], pre['substrate_y'], pre['is_blank']
    # for diagnostics, recompute both options to log which won
    a = _seg_one(roi); b = _seg_one(255 - roi)
    used_inv = _score(b) > _score(a)
    mask = b if used_inv else a
    if blank: mask = np.zeros_like(roi)
    fr = mask.sum()/max(1,mask.size); sol = solidity(mask)
    print(f"{nm:16s} {yb:4d} {str(blank)[0]}    {fr:6.4f} {sol:5.2f} {str(used_inv)[0]:>4s}")
...
cv.imwrite(os.path.join(os.path.dirname(__file__),"v6_overlays.jpg"), mont, [cv.IMWRITE_JPEG_QUALITY, 75])
print("saved v6_overlays.jpg")
