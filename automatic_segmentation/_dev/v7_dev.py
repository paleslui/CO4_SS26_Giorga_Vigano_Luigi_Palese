...
def _seg_one(img):
    ...
    m = cv.morphologyEx(m, cv.MORPH_CLOSE, np.ones((11,11), np.uint8))   # bridge rim gaps
    m = fill_holes(m)                                                     # fill caustics/reflections
    m = cv.morphologyEx(m, cv.MORPH_CLOSE, np.ones((9,9), np.uint8))     # smooth concavities
    m = cv.morphologyEx(m, cv.MORPH_OPEN, np.ones((5,5), np.uint8))      # drop slivers
...
def _score(m):
    ...
    s = solidity(m)
    if s < 0.55: return -1.0                                              # reject ragged blobs
    return s * 10 + min(a, 0.2)
...
def segment_roi(roi, is_blank=False):
    ...
    if sa < 0 and sb < 0:
        return np.zeros_like(roi)         # both polarities fail -> honest blank
    return a if sa >= sb else b

names=["h2o-teflon","h2o-vetro","h2o-metall","h2o-print-5","h2o-plexig","h2o-rain-100","h2o-fuoc","h2o-tessuto"]
tiles=[]
print(f"{'image':16s} {'yb':>4s} blank {'area':>6s} {'solid':>5s} {'inv?':>4s} verdict")
for nm in names:
    ...
    a = _seg_one(roi); b = _seg_one(255 - roi)
    sa, sb = _score(a), _score(b)
    if blank: mask = np.zeros_like(roi); verdict = "BLANK"; used_inv = False
    elif sa < 0 and sb < 0: mask = np.zeros_like(roi); verdict = "REJECTED"; used_inv = False
    else:
        used_inv = sb > sa
        mask = b if used_inv else a
        verdict = "ok"
    fr = mask.sum()/max(1,mask.size); sol = solidity(mask)
    print(f"{nm:16s} {yb:4d} {str(blank)[0]}    {fr:6.4f} {sol:5.2f} {str(used_inv)[0]:>4s} {verdict}")
    bgr = cv.imread(path); bgr_crop = pp.crop_timestamp(bgr); bgr_roi = bgr_crop[:yb, :].copy()
...
cv.imwrite(os.path.join(os.path.dirname(__file__),"v7_overlays.jpg"), mont, [cv.IMWRITE_JPEG_QUALITY, 75])
print("saved v7_overlays.jpg")
