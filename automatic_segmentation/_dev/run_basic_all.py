...
from basic_segment import segment
...
def solidity(m):
    ...
    a, hull = cv.contourArea(c), cv.contourArea(cv.convexHull(c))
...
for path in files:
    ...
    img = cv.imread(path)
    mask, baseline = segment(img)
    area = (mask > 0).mean()
    sol = solidity(mask)
    verdict = "ok" if mask.sum() > 0 else "empty"
    results.append((nm, baseline, area, sol, verdict))
    ...
    TW = 260
    s = TW / ov.shape[1]
    tile = cv.resize(ov, (TW, int(ov.shape[0] * s)))
    lab = np.zeros((22, TW, 3), np.uint8)
    color = (0, 255, 0) if verdict == "ok" else (0, 80, 255)
    cv.putText(lab, f"{nm}|{verdict}", (3, 15), cv.FONT_HERSHEY_SIMPLEX, 0.42, color, 1)
    tiles.append(np.vstack([lab, tile]))

# summary
print(f"\n{'image':22s} {'verdict':8s} {'area':>6s} {'solid':>5s}")
for r in results:
    print(f"{r[0]:22s} {r[4]:8s} {r[2]:6.3f} {r[3]:5.2f}")
...
cv.imwrite(os.path.join(os.path.dirname(__file__), "basic_all_overlays.jpg"), mont, [cv.IMWRITE_JPEG_QUALITY, 72])
print(f"saved basic_all_overlays.jpg ({mont.shape[1]}x{mont.shape[0]})")
