...
from sam_segment_v2 import segment as sam_segment, _get_predictor
...
print("loading SAM..."); t0 = time.time(); _get_predictor(); print(f" loaded {time.time()-t0:.1f}s")
...
for path in files:
    ...
    print(f"  {nm:22s} {v:8s} area={area:.3f} sol={sol:.2f}")
...
cv.imwrite(os.path.join(os.path.dirname(__file__), "sam_v2_all_overlays.jpg"), mont, [cv.IMWRITE_JPEG_QUALITY, 72])
print(f"saved sam_v2_all_overlays.jpg ({mont.shape[1]}x{mont.shape[0]})")
