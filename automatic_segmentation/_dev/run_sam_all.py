...
from sam_segment import segment as sam_segment, _get_predictor
...
t0 = time.time(); _get_predictor(); print(f"  loaded in {time.time()-t0:.1f}s")
...
for path in files:
    ...
    img = cv.imread(path)
    t0 = time.time()
    mask, baseline = sam_segment(img)
    dt = time.time() - t0
    area = (mask > 0).mean(); sol = solidity(mask)
    v = "ok" if mask.sum() > 0 else "empty"
    results.append((nm, baseline, area, sol, v, dt))
    ...
    print(f"  {nm:22s} {v:8s} area={area:.3f} sol={sol:.2f} ({dt:.1f}s)")
...
cv.imwrite(os.path.join(os.path.dirname(__file__), "sam_all_overlays.jpg"), mont, [cv.IMWRITE_JPEG_QUALITY, 72])
print(f"saved sam_all_overlays.jpg ({mont.shape[1]}x{mont.shape[0]})")
